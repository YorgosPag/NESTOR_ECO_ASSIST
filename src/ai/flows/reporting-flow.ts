...
const generateReportFlow = ai.defineFlow(
  ...
  async (query) => {
    ...
    const finalResponse = await ai.generate({
        ...
        prompt: 'Based on the user query and the data you fetched, generate the final response. If a chart was requested, use the provided schema. Otherwise, generate a user-friendly response in Greek. Format it nicely for readability.',
        model: 'googleai/gemini-2.0-flash',
        output: { schema: ReportOutputSchema }
    });
    return finalResponse.output!;
